...
void EnemySpawn(Room::pRL stage){
   int i = 0;
	if(stage->roomID>=2 && stage->roomID<=3){
		for(i=0; i<2; i++){
			Enemy *enemy = new Enemy(rand()%29+1,rand()%29+1,'#',win,'E');
			enemy->display_enemy();
			enemyarray[i]=enemy;
		}	
	}
	...
	if(stage->roomID>5 && stage->roomID<=10){
		...
		for(int j=0; j<2; j++){
			Enemy *enemy = new Enemy(rand()%29+1,rand()%29+1,'#',win,'M');
			enemy->display_enemy();
			enemyarray[i]=enemy;
		}	
	}
	...
	if(stage->roomID>15 && stage->roomID<=20){
		for(int i=0; i<2; i++){
			...
			enemyarray[i]=enemy;
		        ...
		        Enemy *enemy = new Enemy(rand()%29+1,rand()%29+1,'#',win,'H');
		        enemy->display_enemy();
		        enemyarray[i]=enemy;
	}
...
do{
	P->getinput();
	if(P->getinput() == 's'){
		Bullet sh = Bullet(RoomWin, P->getX()+1, P->getY(), '-', 10, enemyarray);
